...
{
	// CREATE FUNCTION TO OPEN & READ FILES
	std::ifstream readFile;
	readFile.open( "Resources/res/Highscore.txt");
	...
	std::ofstream writeFile("Resources/res/Highscore.txt");
	...
	this->_data->assets.LoadTexture("Platinium Medal", PLATINUM_MEDAL_FILEPATH );
	...
	_scoreText.setPosition( _data->window.getSize().x / 10 * 7.25f, _data->window.getSize().y / 2.15 );
	...
	_highScoreText.setPosition(_data->window.getSize().x / 10 * 7.25f, _data->window.getSize().y / 1.78 );

	if( _score >= PLATINIUM_MEDAL_SCORE )
